    ...
    internal sealed class ImGuiController : IDisposable
    {
        ...
        public void CreateDeviceResources(GraphicsDevice gd, OutputDescription outputDescription)
        {
            ...
            var fragmentShaderBytes =
                LoadEmbeddedShaderCode(gd.ResourceFactory, "imgui-frag", ShaderStages.Fragment);
        ...
        private void RemoveImGuiBinding(TextureView textureView)
        {
            if (!_setsByView.TryGetValue(textureView, out var rsi)) return;
        ...
        {
            if (!_autoViewsByTexture.TryGetValue(texture, out var textureView)) return;
        ...
        private void UpdateImGuiInput(InputSnapshot snapshot, IntPtr handle)
        {
            ...
            // ! 6 more instructions are being called if we use foreach, so skip it.
            ...
            for (var i = 0; i < keyCharPresses.Count; i++) io.AddInputCharacter(keyCharPresses[i]);
            ...
            NativeMethods.SetOverlayClickable(handle, io.WantCaptureKeyboard || io.WantCaptureMouse);
        }
